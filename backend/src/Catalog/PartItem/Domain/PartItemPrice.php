...
final class PartItemPrice extends FloatValueObject
{
    ...
    private function ensureIsValid(float $value): void
    {
        if ($value < self::MIN_PRICE) {
            throw new InvalidArgumentException('Part Item price cannot be negative.');
        }

        if ($value > self::MAX_PRICE) {
            throw new OutOfBoundsException(
                sprintf('Part Item price cannot exceed %.2f.', self::MAX_PRICE)
            );
        }
    }
}